...
if [ ! -f /etc/openldap/CONFIGURED ]; then
    ...
    then
        ...
        #cp -r /opt/openshift/config/*.conf /etc/openldap
        #cp -r /opt/openshift/config/schema /etc/openldap
        #cp -r /opt/openshift/config/slapd.d /etc/openldap
        ...
        # configure certs - we will remove the existing "OpenLDAP Server" cert from the db and generate our
        # own self-signed versions.  next iteration will import these from a shared folder...

        #delete the OpenLDAP Server cert
        #certutil -d /etc/openldap/certs -D -n "OpenLDAP Server"

        #create a new certificate database
        #certutil -N -d /etc/openldap/certs -p /etc/openldap/certs/password

        #add in the other certs from external source (mount secret containing cacert.pk12, openldapcert.pk12 and pk12password to "/tmp/certs/pk12")
        #pk12util -i /tmp/certs/pk12/cacert.pk12 -k /etc/openldap/certs/password -w /tmp/certs/pk12/pk12password -d /etc/openldap/certs
        #pk12util -i /tmp/certs/pk12/openldapcert.pk12 -k /etc/openldap/certs/password -w /tmp/certs/pk12/pk12password -d /etc/openldap/certs
        
        # this is for generating your own self signed certs if you want to...        
        #certutil -S -n "CA certificate" -s "cn=CAcert" -x -t "CT,," -m 1000 -v 120 -d /etc/openldap/certs -z /etc/openldap/certs/noise.txt -f /etc/openldap/certs/password
        #certutil -S -n "OpenLDAP Server" -s "cn=FQDN,cn=Directory Server" -c "CA certificate" -t "CTu,u,u" -m 1001 -v 120 -d /etc/openldap/certs -z /etc/openldap/certs/noise.txt -f /etc/openldap/certs/password
        ...
        #echo "adding activemq openldap schema"
    	#ldapadd -Y EXTERNAL -H ldapi:/// -f /usr/local/etc/openldap/activemq-openldap.ldif -d $OPENLDAP_DEBUG_LEVEL
        ...
        then
            ...
            mv -f /opt/openshift/config/*.conf /etc/openldap
            mv -f /opt/openshift/config/schema /etc/openldap/schema
            mv -f /opt/openshift/config/slapd.d /etc/openldap/slapd.d
            mv /tmp/certs/*.pem /etc/openldap/certs 
        else
